...
function connect() {
  ...
  ws.onopen = () => {
    ...
    ws.send(JSON.stringify({ type: 'handshake', extensionId: chrome.runtime.id, version: manifest.version }));
  };
  ...
  ws.onclose = () => {
    ...
    if (reconnectAttempts === 1) {
      console.log(`[bridge] ${wasConnected ? 'disconnected' : 'server not running'}, will retry silently`);
...
async function handleCommand(msg) {
  ...
  switch (action) {
    ...
    case 'dom.evaluateHandle': {
      ...
      const ehResult = await executeHandleInPage(tabId, params.fn, params.args || [], params.elementMarkers || []);
      ...
      if (ehResult.__type === 'element' && ehResult.marker) {
        const handleId = await forwardToContentScript(tabId, 'dom.registerMarkedElement', { marker: ehResult.marker });
      ...
      if (ehResult.__type === 'object' && ehResult.properties) {
        ...
        for (const [key, prop] of Object.entries(ehResult.properties)) {
          if (prop.__type === 'element' && prop.marker) {
            const handleId = await forwardToContentScript(tabId, 'dom.registerMarkedElement', { marker: prop.marker });
    ...
    case 'dom.elementEvaluate': {
      ...
      try {
        ...
        try {
          ...
          forwardToContentScript(tabId, 'dom.unmarkElement', { markerId }).catch(() => {});
          throw err;
        }
      }
    }
...
async function executeInPage(tabId, fnString, args, markerId) {
  ...
  try {
    const result = await forwardToContentScript(tabId, 'dom.evaluateViaScript', {
      fn: fnString, args: args || [], markerId: markerId || null,
    });
    return result;
  } catch (scriptErr) {
    ...
    chrome.tabs.sendMessage(tabId, payload, opts, async (response) => {
      if (chrome.runtime.lastError) {
        ...
        chrome.tabs.sendMessage(tabId, payload, opts, (retryResponse) => {
          ...
          if (!retryResponse) { reject(new Error('No response from content script')); return; }
          if (retryResponse.error) { reject(new Error(retryResponse.error)); return; }
  ...
  (details) => {
    if (ws && ws.readyState === WebSocket.OPEN && details.tabId > 0) {
      ws.send(JSON.stringify({
        type: 'event', event: 'response',
        data: { url: details.url, status: details.statusCode, tabId: details.tabId, method: details.method }
      }));
    }
  },
  { urls: ['<all_urls>'] }
