  ...
  function send(action, params = {}, tabId = null) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Command ${action} timed out (30s)`));
      }, 30000);
      ...
      if (action.startsWith('human.')) {
        ...
        if (humanConfig.avoid) {
          ...
          params.avoid = {
            selectors: [...(globalAvoid.selectors || []), ...(reqAvoid.selectors || [])],
            classes: [...(globalAvoid.classes || []), ...(reqAvoid.classes || [])],
            ids: [...(globalAvoid.ids || []), ...(reqAvoid.ids || [])],
            attributes: { ...(globalAvoid.attributes || {}), ...(reqAvoid.attributes || {}) },
          };
        }
      }

      // Push framework runtime config into content-script bound commands.
      if (config.framework && (action.startsWith('dom.') || action.startsWith('human.'))) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (data) => {
      ...
      if (msg.type === 'handshake') {
        isExtension = true;
        extensionWs = ws;
        log('INFO', `Extension connected (id: ${msg.extensionId}, v${msg.version || '?'})`);
        ...
        if (msg.extensionId && config.profile) {
          try {
            ...
            const profileDir = (config.profile || '').replace(/^~/, os.homedir());
